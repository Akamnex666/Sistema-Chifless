...
export class GrokProvider implements LLMProvider {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.modelName = this.configService.get<string>('GROK_MODEL') || 'grok-2-latest';
    
    if (!this.apiKey) {
      this.logger.warn('GROK_API_KEY no está configurada');
    }
  }
  ...
  async generateResponse(
    ...
    try {
      ...
      this.logger.debug(`Enviando petición a Grok: ${this.modelName}`);
      ...
      if (!response.ok) {
        ...
        this.logger.error(`Error de Grok API: ${response.status} - ${errorBody}`);
      ...
      this.logger.debug(`Respuesta de Grok recibida: ${llmResponse.text.substring(0, 100)}...`);

      return llmResponse;
    } catch (error) {
      this.logger.error('Error al generar respuesta con Grok:', error);
      throw error;
    }
  }
